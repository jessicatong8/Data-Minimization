...
def purposes2(in_file, categories, out_file):
    """
    :param in_files: (list) list of filenames (strings) for json files with app info
    :param categories: (list) list of category names (strings) corresponding to files
    :return: (df) dataframe containing number of apps collecting data for each purpose by category
...
in_files = ["ios_apps.json", "ios_games.json", "ios_finance.json", "ios_shopping.json", "ios_photovideo.json", "ios_fooddrink.json"]
top_categories = ["All Apps", 'Games', 'Finance', 'Shopping', 'Photo & Video', 'Food & Drink']

categories = load_json("ios_categories_ratings.json")
categories = list(categories.keys())
#print(categories)

#print(purposes(in_files, top_categories, "ios_purposes_rating.csv"))
print(purposes2("ios_apps.json", categories, "ios_purposes_allcategories.csv"))
